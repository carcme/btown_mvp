...
public class ShareDialog {
    ...
    public static void sendQRCode(final Activity activity, String encodeType, Bundle encodeData, String strEncodeData) {
        ...
        if (resolved != null) {
            ...
            builder.setPositiveButton(activity.getString(android.R.string.ok), new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(IntentUtils.getUrlWithRef(ZXING_BARCODE_SCANNER_COMPONENT)));
                    try {
                        activity.startActivity(intent);
    ...
    public static void sendToClipboard(Context context, String text) {
        ...
        Toast.makeText(context, R.string.shared_string_copied_to_clipboard, Toast.LENGTH_SHORT).show();
    }
}