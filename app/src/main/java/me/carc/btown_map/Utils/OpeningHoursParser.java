...
public class OpeningHoursParser {
    ...
    public static class OpeningHours implements Serializable {
        ...
        private String originalFormatted;
        ...
        public void setOriginalFormatted(String originalFormatted) {
            this.originalFormatted = originalFormatted;
        }
...
//        String[] rules = format.split(";"); //$NON-NLS-1$
        String[] rules = buildRuleArray(format);

        // FIXME: What if the semicolon is inside a quoted string?
        OpeningHours oh = new OpeningHours();
        oh.setOriginal(format);
        oh.setOriginalFormatted(buildOriginalFormatted(rules));
        for (String r : rules) {
            r = r.trim();
            if (r.length() == 0) {
                continue;
    ...
    public static String buildOriginalFormatted(String[] rules) {
        StringBuilder times = new StringBuilder();
        for (int i = 0; i < rules.length; i++) {
            times.append(rules[i].trim());
            if (i != (rules.length - 1))
                times.append(", ");
        }
    ...
    /**
     * Allow for "additional_rule_separator" (http://wiki.openstreetmap.org/wiki/Key:opening_hours/specification)
     * @param format
     * @return
     */
    private static String[] buildRuleArray(String format) {
        ArrayList<String> array = new ArrayList<>();
        String[] rules = format.split(";");

        for (String rule : rules) {
            String[] splitter = rule.split(",");
            Collections.addAll(array, splitter);
        }

        String[] retValue = new String[array.size()];
